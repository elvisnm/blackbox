...
4. **Read the ticket** — understand the Goal, Context, Requirements, and Constraints before starting design work.

5. **Set the ticket status** to "Designing" in Asana.

6. **Research existing UI patterns** in the codebase (read-only):
...
7. **Fill the UI/UX section** of the ticket:
...
8. **Present the UI/UX spec** to the Designer:
...
9. **Update the Asana ticket** after Designer approval:
...
10. **When design is complete**, ask the Designer if the ticket is ready:
